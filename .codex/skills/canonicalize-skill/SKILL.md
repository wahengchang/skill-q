...
3. 確認以下資訊：
   ...
   - 何時應觸發，以及何時不應觸發；
   - 所需輸入、預期輸出、必要工具或支援檔；
   - 符合規範、具體且不易撞名的 kebab-case 名稱，必須以 `y-` 開頭（skill-q 的技能一律 `y-<name>`）。
...
5. 草擬 `commands-src/<name>/SKILL.md`，格式必須是：
   ...
   資料夾名必須與 `name` 完全相同。frontmatter 只放 `name` 與 `description`；名稱使用小寫 kebab-case 並以 `y-` 開頭。skill-q 的技能要 quick：單一用途、指令精簡，不要塞入多階段流程。必要時重寫、去除矛盾並補上安全界線，不要原封不動包裝含糊的 raw skill。
