...
REPO_SIGNATURE="skill-q repository"
...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/skill-q-uninstall.XXXXXX")
trap 'rm -rf "$TMP"' EXIT
...
Removes the current skill-q installation plus historical deployments. Historical
...
checkout_has_local_content() {
  ...
  for entry in "${TRANSFORMED_TARGETS[@]}"; do disposable+=("${entry##*:}"); done
  while IFS= read -r -d '' path; do
    skip=0
    for allowed in "${disposable[@]}"; do
      allowed=${allowed%/}
      if [[ "$path" == "$allowed" || "$path" == "$allowed/"* ]]; then skip=1; break; fi
    done
    (( skip )) || return 0
...
if (( ! assume_yes )) && [[ -t 0 && -t 1 ]]; then
  reply=""
  printf 'Remove skill-q deployments for %s, including historical skills? [y/N] ' "$selected"
  read -r reply || reply=""
  [[ "$reply" == [yY]* ]] || { echo 'Uninstall declined; nothing changed.'; exit 0; }
fi

legacy_roots=("$ROOT")
if [[ -n "$RECORDED_ROOT" && "$RECORDED_ROOT" != "$ROOT" ]]; then legacy_roots+=("$RECORDED_ROOT"); fi
...
historical_skill_path_owned() {
  ...
  if [[ -L "$path" ]]; then
    ...
    linked=$(readlink "$path") || return 1
    case "$linked" in /*) target=$linked ;; *) target="$(dirname "$path")/$linked" ;; esac
    if [[ -e "$path" ]]; then
      repo_skill_copy "$target/SKILL.md" && return 0
      return 1
    fi
    case "$linked" in */commands/"$name"|*/commands-src/"$name") return 0 ;; esac
    return 1
  fi
...
historical_command_link_owned() {
  ...
  case "$linked" in /*) target=$linked ;; *) target="$(dirname "$path")/$linked" ;; esac
  if [[ -e "$path" ]]; then
    grep -Fq "$SKILL_Q_MANAGED_MARKER" "$target" 2>/dev/null && return 0
    return 1
  fi
  case "$linked" in */opencode-commands/"$name".md) return 0 ;; esac
  return 1
}
  ...
  if [[ -d "$commands_dir" ]]; then
    while IFS= read -r -d '' path; do
      if [[ -L "$path" ]]; then
        ...
        grep -Fq "$SKILL_Q_MANAGED_MARKER" "$path" 2>/dev/null || continue
      else
        continue
      fi
      rm -f "$path"
