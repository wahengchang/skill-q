...
Removes the current skill-q installation plus legacy skill/command links that
...
if (( ! assume_yes )) && [[ -t 0 && -t 1 ]]; then
  reply=""
  printf 'Remove skill-q deployments for %s, including legacy links? [y/N] ' "$selected"
  read -r reply || reply=""
  ...
  if [[ -d "$commands_dir" ]]; then
    while IFS= read -r -d '' path; do
      if [[ -L "$path" ]]; then
        ...
        grep -q "$SKILL_Q_MANAGED_MARKER" "$path" 2>/dev/null || continue
      else
        continue
      fi
      rm -f "$path"
