...
# call skill_q_test_finish at the end of the suite.
#
...
SKILL_Q_TEST_SUITE=${SKILL_Q_TEST_SUITE:-full}
# Seconds a single test may run before it is terminated as a failure.
SKILL_Q_TEST_TIMEOUT=${SKILL_Q_TEST_TIMEOUT:-240}
...
_skill_q_test_index=0
_skill_q_test_timings=()

_skill_q_validate_timeout() {
  [[ $SKILL_Q_TEST_TIMEOUT =~ ^[1-9][0-9]*$ ]] || {
    echo "SKILL_Q_TEST_TIMEOUT must be a whole number of seconds, got: $SKILL_Q_TEST_TIMEOUT" >&2
    exit 2
  }
}
_skill_q_validate_timeout
...
skill_q_test_parse_args() {
  while (( $# )); do
    case $1 in
      --fast) SKILL_Q_TEST_SUITE=fast ;;
      --full) SKILL_Q_TEST_SUITE=full ;;
      --timeout)
        shift
        [[ $# -gt 0 ]] || { echo 'missing value for --timeout' >&2; exit 2; }
        SKILL_Q_TEST_TIMEOUT=$1
        ;;
      *)
...
skill_q_test_format_ms() {
  local ms=$1
  printf '%d.%02ds\n' "$((ms / 1000))" "$(((ms % 1000) / 10))"
}

_skill_q_suite_started=$(skill_q_test_now_ms)
...
skill_q_test_ensure_template() {
  [[ -n ${SKILL_Q_TEST_TEMPLATE:-} ]] && return 0
  SKILL_Q_TEST_TEMPLATE="$TEST_ROOT/.template"
  mkdir -p "$SKILL_Q_TEST_TEMPLATE"
  cp -a "$PROJECT_ROOT/." "$SKILL_Q_TEST_TEMPLATE/"
  rm -rf \
    "$SKILL_Q_TEST_TEMPLATE/.git" \
    "$SKILL_Q_TEST_TEMPLATE/commands" \
    "$SKILL_Q_TEST_TEMPLATE/opencode-commands"
  # Agent-local state is not part of a fresh repository fixture: carrying it in
  # makes fixtures depend on the developer's own checkout, and untracked
  # ignored content blocks `uninstall --remove-checkout`.
  rm -rf "$SKILL_Q_TEST_TEMPLATE/.claude"

  # The suite must not depend on which skills this repository happens to ship:
  # a fork that deletes the samples and adds its own skills still has to pass.
  # These two names are the stable fixtures long-lived regression cases build,
  # sync and update against, so synthesize them whenever they are absent.
  local fixture
  for fixture in example-skill funny-text-rewriter; do
    [[ -f "$SKILL_Q_TEST_TEMPLATE/commands-src/$fixture/SKILL.md" ]] && continue
    mkdir -p "$SKILL_Q_TEST_TEMPLATE/commands-src/$fixture"
    cat > "$SKILL_Q_TEST_TEMPLATE/commands-src/$fixture/SKILL.md" <<EOF
---
name: $fixture
...
skill_q_test_ensure_built_template() {
  [[ -n ${SKILL_Q_TEST_BUILT_TEMPLATE:-} ]] && return 0
  skill_q_test_ensure_template
  local built="$TEST_ROOT/.template-built"
  mkdir -p "$built"
  cp -a "$SKILL_Q_TEST_TEMPLATE/." "$built/"
  "$built/bin/build.sh" >/dev/null
  SKILL_Q_TEST_BUILT_TEMPLATE=$built
}
...
copy_project() {
  ...
  skill_q_test_ensure_template
  mkdir -p "$destination"
  cp -a "$SKILL_Q_TEST_TEMPLATE/." "$destination/"
}
...
copy_built_project() {
  ...
  skill_q_test_ensure_built_template
  mkdir -p "$destination"
  cp -a "$SKILL_Q_TEST_BUILT_TEMPLATE/." "$destination/"
}
...
run_test() {
  ...
  if [[ $SKILL_Q_TEST_SUITE == fast && $in_fast_suite -eq 0 ]]; then
    skipped=$((skipped + 1))
    return 0
  fi
  ...
  _skill_q_test_index=$((_skill_q_test_index + 1))
  local marker="$TEST_ROOT/.timeout-marker.$_skill_q_test_index"
  local finished="$TEST_ROOT/.test-finished.$_skill_q_test_index"
  rm -f "$marker" "$finished"
  local start elapsed status=0
  start=$(skill_q_test_now_ms)
  ...
  (
    waited=0
    while (( waited < SKILL_Q_TEST_TIMEOUT )); do
      [[ -e $finished ]] && exit 0
    ...
    done < <(_skill_q_test_descendant_pids "$pid")
  ...
  elapsed=$(($(skill_q_test_now_ms) - start))
  _skill_q_test_timings+=("$elapsed $name")

  if [[ -e $marker ]]; then
    printf 'TIMEOUT after %ss\n' "$SKILL_Q_TEST_TIMEOUT"
  ...
  if (( status == 0 )); then
    printf 'PASS (%s)\n' "$(skill_q_test_format_ms "$elapsed")"
    passed=$((passed + 1))
  else
    printf 'FAIL (%s)\n' "$(skill_q_test_format_ms "$elapsed")"
    failed=$((failed + 1))
...
skill_q_test_finish() {
  local label=${1:-suite}
  local total=0
  [[ -n $_skill_q_suite_started ]] && total=$(($(skill_q_test_now_ms) - _skill_q_suite_started))

  if (( ${#_skill_q_test_timings[@]} > 0 )); then
    printf '\nSlowest cases in %s:\n' "$label"
    local line ms name
    while IFS= read -r line; do
      ms=${line%% *}
      name=${line#* }
      printf '  %8s  %s\n' "$(skill_q_test_format_ms "$ms")" "$name"
    done < <(printf '%s\n' "${_skill_q_test_timings[@]}" | sort -rn | head -5)
  fi

  printf '\nRESULT [%s | %s suite]: %d passed, %d failed' \
    "$label" "$SKILL_Q_TEST_SUITE" "$passed" "$failed"
  (( timed_out > 0 )) && printf ' (%d timed out)' "$timed_out"
  (( skipped > 0 )) && printf ', %d skipped (full suite only)' "$skipped"
  printf ' in %s\n' "$(skill_q_test_format_ms "$total")"
