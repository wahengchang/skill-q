...
repo_url=$1
ref=$2
MANAGED_MARKER="skill-q-managed-command"

tmp=$(mktemp -d "${TMPDIR:-/tmp}/skill-q-cloud.XXXXXX")
trap 'rm -rf "$tmp"' EXIT
...
# happens to live under a skill-q checkout is not enough to claim ownership.
