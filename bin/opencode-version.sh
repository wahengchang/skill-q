...
requested=${SKILL_Q_OPENCODE_VERSION:-auto}
case "$requested" in
  v1|1) echo v1; exit 0 ;;
  v2|2) echo v2; exit 0 ;;
  auto) ;;
  *)
    echo "Unknown SKILL_Q_OPENCODE_VERSION: $requested (expected auto, v1, or v2)" >&2
    exit 2
    ;;
esac
...
if [[ -z "$major" ]]; then
  echo "Could not detect the OpenCode version; assuming v1. Set SKILL_Q_OPENCODE_VERSION=v1 or v2 to decide explicitly." >&2
  echo v1
elif (( major >= 2 )); then
  echo v2
else
