...
skill_q_validate_artifact_paths() {
...
skill_q_ensure_artifacts_ignored() {
  ...
  for path in "$CANONICAL_DEST" "${TRANSFORMED_TARGETS[@]}"; do
    [[ "$path" == *:* ]] && path=${path#*:}
    pattern="/$path/"
    if ! git -C "$ROOT" check-ignore -q --no-index "$path/.skill-q-ignore-probe" 2>/dev/null; then
      printf '\n# skill-q disposable artifact\n%s\n' "$pattern" >> "$exclude"
    fi
    git -C "$ROOT" check-ignore -q --no-index "$path/.skill-q-ignore-probe" || {
