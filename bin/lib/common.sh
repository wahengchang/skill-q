#!/usr/bin/env bash
# Shared helpers for the skill-q lifecycle CLI. Sourced, never executed.
#
# Everything that has to agree between `bin/skill-q`, the compatibility
...
SKILL_Q_LIB_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
SKILL_Q_ALL_AGENTS=(claude codex opencode)
SKILL_Q_MANAGED_MARKER="skill-q-managed-command"
SKILL_Q_MANIFEST_SCHEMA=1

sx_die() { printf 'skill-q: %s\n' "$*" >&2; exit 1; }
sx_warn() { printf 'skill-q: %s\n' "$*" >&2; }
...
sx_is_agent() { sx_contains "$1" "${SKILL_Q_ALL_AGENTS[@]}"; }
...
# SKILL_Q_CODEX_COMPAT=0 to drop the compatibility path.
sx_agent_skill_dirs() {
  case "$1" in
    claude) printf '%s\n' "$HOME/.claude/skills" ;;
    codex)
      printf '%s\n' "$HOME/.agents/skills"
      [[ "${SKILL_Q_CODEX_COMPAT:-1}" == 0 ]] || printf '%s\n' "$HOME/.codex/skills"
      ;;
...
sx_parse_agents() {
  ...
  for token in $raw; do
    [[ -n "$token" ]] || continue
    sx_is_agent "$token" || sx_die "unknown agent: $token (expected ${SKILL_Q_ALL_AGENTS[*]})"
    selected+=("$token")
  done
  for agent in "${SKILL_Q_ALL_AGENTS[@]}"; do
...
sx_state_root() {
  local base=${SKILL_Q_STATE_HOME:-${XDG_STATE_HOME:-$HOME/.local/state}}
  printf '%s/skill-q\n' "$base"
}

# State directory for one installation. SKILL_Q_STATE_DIR pins it explicitly
# (used by tests and by anyone who kept the pre-lifecycle layout).
sx_state_dir() {
  if [[ -n "${SKILL_Q_STATE_DIR:-}" ]]; then
    printf '%s\n' "$SKILL_Q_STATE_DIR"
    return
  fi
  printf '%s/%s\n' "$(sx_state_root)" "$1"
...
sx_all_manifests() {
  local dir
  if [[ -n "${SKILL_Q_STATE_DIR:-}" ]]; then
    [[ -f "$SKILL_Q_STATE_DIR/install.json" ]] && printf '%s\n' "$SKILL_Q_STATE_DIR/install.json"
    return 0
  fi
  local root
  root=$(sx_state_root)
...
sx_install_id() {
  ...
  if git_dir=$(sx_git_dir "$root"); then
    id_file="$git_dir/skill-q-install-id"
    if [[ -f "$id_file" ]]; then
...
sx_manifest_scalar() {
  [[ -f "$1" ]] || return 0
  awk -f "$SKILL_Q_LIB_DIR/manifest.awk" -v mode=scalar -v key="$2" "$1"
}

sx_manifest_array() {
  [[ -f "$1" ]] || return 0
  awk -f "$SKILL_Q_LIB_DIR/manifest.awk" -v mode=array -v key="$2" "$1"
}

sx_manifest_map() {
  [[ -f "$1" ]] || return 0
  awk -f "$SKILL_Q_LIB_DIR/manifest.awk" -v mode=map -v key="$2" "$1"
}

# agent<TAB>kind<TAB>path<TAB>target, one managed entry per line.
sx_manifest_entries() {
  [[ -f "$1" ]] || return 0
  awk -f "$SKILL_Q_LIB_DIR/manifest.awk" -v mode=entries "$1"
}
